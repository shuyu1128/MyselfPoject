  ...
  <div class="details">
    ...
    <div class="right-sou">
      <div class="basic">
        ...
        <div>
          <p @click="zidingyi" class="fDetails-step5">
      ...
      <div style="position: relative;">
        <router-link :to="{ name: '详情', params: { id:changeid}}">
          <el-button
            type="primary"
        ...
        <el-tabs
          v-model="$store.state.activeNames"
        ...
        >
          <el-tab-pane lazy label="个人信息" key="1" name="1">
            <div class="tap-data">
              <Gerenfo v-if="showGerenfo" :did="changeid" />
            </div>
          </el-tab-pane>
          <el-tab-pane lazy label="在院信息" key="2" name="2">
            <div class="tap-data">
              <Zaiyuan v-if="showzaiyuan" :did="changeid" />
            </div>
          </el-tab-pane>
          <el-tab-pane lazy label="护理方案" key="3" name="3">
            <div class="tap-data">
              <Program v-if="changeid!=''" :did="changeid" />
            </div>
          </el-tab-pane>
          <el-tab-pane lazy label="电子围栏" key="4" name="4">
            <div class="tap-data">
              <personalEleFence v-if="weilanshow" :did="changeid" :changeBtn="true" />
            </div>
          </el-tab-pane>
          <el-tab-pane lazy label="智能设备" key="5" name="5">
            <div class="tap-data">
              <Equipment v-if="showzaiyuan" :did="changeid" />
            </div>
          </el-tab-pane>
          <el-tab-pane lazy label="附件管理" key="6" name="6">
            <div class="tap-data">
              <Enclosure v-if="changeid!=''" show="bianji" :did="changeid" />
            </div>
          </el-tab-pane>
        </el-tabs>
      </div>
...
export default {
  ...
  watch: {
    changeid(val) {
      if (val != undefined) {
        this.showzaiyuan = false;
        this.$nextTick(() => {
          this.showzaiyuan = true;
        });
  ...
  methods: {
    ...
    handleClick(activeName, oldActiveName) {
      //  判断页签到个人和在院时重新获取信息，防止快捷信息卡不刷新
      if (activeName == "1") {
        this.showGerenfo = false;
        this.$nextTick(() => {
          this.showGerenfo = true;
        });
      ...
      if (
        this.$store.state.hospitalshow &&
        !this.$store.state.hospitalshowactive
      ) {
        this.$message.warning("请按照提示进行操作！");
        return false;
      }
      this.$store.state.hospitalshowactive = false;
    },
  ...
  beforeRouteLeave(to, form, next) {
    if (to.path !== "/Detailsx") this.$store.state.activeNames = "1";
...
<style>
.details .el-checkbox.is-bordered + .el-checkbox.is-bordered {
  margin-left: 0px;
}
</style>
<style scoped>
