    ...
    <el-aside id="asideNav">
      ...
      >
        <template v-for="(item,index) in $store.getters.routers" v-if="!item.hidden">
          <el-submenu v-show="!item.alone&&item.children.length>0" :index="index+''">
            <template slot="title">
        ...
        <!-- <el-menu-item @click="zuzhi" v-if="item.name=='长者中心'" index="/Details:id">  
          <span slot="title">入院</span>
        </el-menu-item> -->

          </el-submenu>
          <el-menu-item :index="item.path" v-show="item.alone">
    ...
    selectmenu(key) {
      ...
      let navTitle = function(path, routerARR) {
        for (let i = 0; i < routerARR.length; i++) {
          if (
            routerARR[i].path !== "/login" &&
            routerARR[i].path !== "/index"
          ) {
            if (routerARR[i].children.length > 0) {
              for (var k = 0; k < routerARR[i].children.length; k++) {
                if (routerARR[i].children[k].path === path) {
                  name = routerARR[i].children[k].name;
                }
              }
            }
          }
